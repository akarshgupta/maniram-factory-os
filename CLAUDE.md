...
**Four entry pages**, all standalone HTML with inline or `js/` scripts:
...
- `review.html` — public customer review funnel reached by scanning a QR code (`js/review.js`, no login). 4-5 stars: pick/edit a template comment, which is copied to the clipboard and `GOOGLE_REVIEW_URL` (`js/config.js`) opens in a new tab for the customer to paste and post. 1-3 stars: private feedback only, never shown a Google link — this is a deliberate review-gating choice (owner's call, not a technical default) so a bad experience reaches the owner instead of becoming a public review. Every submission is logged to the Reviews tab (below) via the `saveReview` action. The owner-facing summary, private-feedback list, and printable QR code (rendered client-side with the `qrcodejs` CDN library, pointed at `review.html`'s own deployed URL) live in the main app under Tools → Customer Reviews (`js/reviews-admin.js`).
