// ══════════════════════════════════════════════════════════════
// ANALYTICS.JS — Factory Performance Dashboards
// ══════════════════════════════════════════════════════════════

let _anCharts = {};   // chart instances — destroy before re-render
let _anPeriod = 6;    // months to analyse (3 / 6 / 12)
...
function _renderRevenueChart() {
  ...
  const revenues = months.map(m =>
    _doneOrders().filter(o => o.date?.startsWith(m.key))
                 .reduce((s,o) => s + (o.qty||0)*(o.rate||0), 0) / 1000
  );
  const boxes = months.map(m =>
    _doneOrders().filter(o => o.date?.startsWith(m.key))
...
function _renderTopCustomersChart() {
  ...
  const map = {};
  _doneOrders().forEach(o => {
    map[o.customer] = (map[o.customer] || 0) + (o.qty||0)*(o.rate||0);
  });
  ...
  _anCharts.customers = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: sorted.map(([,v]) => Math.round(v/1000)),
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: c => ` ₹${c.raw}K` } }
      },
...
function _renderStatusDonut() {
  ...
  const colors = {
    'New':'#3B82F6','In Production':'#F59E0B','Ready':'#10B981',
...
// ── Monthly Order Volume (stacked bar) ───────────────────────
...
function _renderVolumeChart() {
  ...
  const months = _lastNMonths(_anPeriod);
  const mkCount = (filterFn) => months.map(m => orders.filter(o => filterFn(o) && o.date?.startsWith(m.key)).length);
  ...
  _anCharts.volume = new Chart(ctx, {
    ...
    data: {
      labels: months.map(m => m.label),
      datasets: [
        { label: 'Completed', data: mkCount(o => ['Dispatched','Delivered'].includes(o.status)),       backgroundColor: '#10B981', borderRadius: 4 },
        { label: 'Active',    data: mkCount(o => !['Dispatched','Delivered','Cancelled'].includes(o.status)), backgroundColor: '#3B82F6', borderRadius: 4 },
        { label: 'Cancelled', data: mkCount(o => o.status === 'Cancelled'),                             backgroundColor: '#F87171', borderRadius: 4 },
      ]
    },
    options: {
      responsive: true,
...
function _renderProductMixChart() {
  ...
  const map = {};
  _doneOrders().filter(o => o.product?.trim()).forEach(o => {
    const k = o.product.trim();
    map[k] = (map[k]||0) + (o.qty||0);
  });
  ...
  _anCharts.products = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: { callbacks: { label: c => ` ${c.raw.toLocaleString('en-IN')} boxes` } }
      },
...
function _renderCustomerTable() {
  ...
  orders.forEach(o => {
    ...
    if (['Dispatched','Delivered'].includes(o.status)) {
      d.revenue += (o.qty||0)*(o.rate||0);
      d.boxes   += (o.qty||0);
    } else if (o.status !== 'Cancelled') {
      d.pipeline += (o.qty||0)*(o.rate||0);
    }
